...
export const initOfflineDB = async () => {
  ...
  // Migration: add batch column to attendance_sessions
  // WHY: practical sessions need to store which batch was marked
  // '' (empty string) = lecture session (no batch)
  // 'A', 'B', 'C'   = practical session for that batch
  try {
    await db_local.execAsync(
      `ALTER TABLE attendance_sessions ADD COLUMN batch TEXT NOT NULL DEFAULT '';`
    );
    console.log('✅ Migration: batch column added to attendance_sessions');
  } catch {
    // Column already exists — safe to ignore
  }
  ...
  try {
    await db_local.execAsync(
      `ALTER TABLE cached_students ADD COLUMN batch TEXT NOT NULL DEFAULT '';`
    );
    console.log('✅ Migration: batch column added to cached_students');
  } catch {
    // Column already exists — safe to ignore
  }
  ...
  try {
    await db_local.execAsync(
      `ALTER TABLE cached_students ADD COLUMN roll_number TEXT NOT NULL DEFAULT '';`
    );
    console.log('✅ Migration: roll_number column added to cached_students');
  } catch {
    // Column already exists — safe to ignore
  }
...
export const saveAttendanceOffline = async (sessionData) => {
  ...
  await db_local.runAsync(
    `INSERT OR REPLACE INTO attendance_sessions
      (local_id, class_id, class_name, subject_id, subject_name, teacher_id, date, records, saved_at, session_type, batch, synced)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)`,
    [
      localId,
      sessionData.classId,
      ...
      sessionData.batch ?? '',
      // WHY empty string default: lecture sessions have no batch
      // practical sessions will have 'A', 'B', or 'C'
    ]
  );
  ...
  for (const session of pending) {
    try {
      ...
      await addDoc(collection(db, 'attendance'), {
        ...
        batch:       session.batch ?? '',
        // WHY include batch: when syncing offline practical sessions to Firestore
        // the batch field must be present so reports can filter correctly
        // lectures will have batch: '' which is fine
...
export const syncTeacherStructureToCache = async (uid) => {
    ...
    const studentList = await Promise.all(
      studentUids.map(async (suid) => {
        ...
        if (uSnap.exists()) {
          ...
          return {
            uid:         suid,
            name:        d.full_name ?? d.name ?? suid,
            batch:       d.batch ?? '',
            roll_number: d.roll_number ?? '',
            // WHY cache roll_number: show real roll numbers in attendance table
            // and sort students in register order when offline
          };
        }
        return { uid: suid, name: suid, batch: '', roll_number: '' };
      })
    );
  ...
  for (const classId of Object.keys(studentsPerClass)) {
    for (const student of studentsPerClass[classId]) {
      await db_local.runAsync(
        `INSERT OR REPLACE INTO cached_students (uid, class_id, name, batch, roll_number)
         VALUES (?, ?, ?, ?, ?)`,
        [student.uid, classId, student.name, student.batch ?? '', student.roll_number ?? '']
      );
    }
  }
...
export const getCachedStudents = async (classId) => {
  ...
  return rows.map(r => ({
    ...
    // WHY include roll_number: AttendanceScreen shows real roll numbers
    // and sorts students in register order
  }));
};
