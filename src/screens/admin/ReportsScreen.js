  ...
  const calculateDefaulters = async () => {
    ...
    try {
      ...
      const results = studentUids.map(uid => {
        // Count present in lecture sessions
        ...
        // Count present in practical sessions
        ...
        const practicalPct = pct(practicalPresent, totalPracticals);

        // Overall = average of available percentages
        // If only lectures exist → use lecture % only
        // If only practicals exist → use practical % only
        ...
        if (lecturePct !== null && practicalPct !== null) {
          ...
          overallPct = null; // no sessions at all
        }
      ...
      // Only show defaulters + sort by overall % ascending
          ...
          {defaulters !== null && (
            <View style={s.card}>
              ...
              <View style={s.resultHeader}>
                ...
                {defaulters.length > 0 && (
                  <TouchableOpacity
                    style={[s.pdfBtn, exporting && s.pdfBtnDisabled]}
                    onPress={exportPDF}
                    disabled={exporting}
                    activeOpacity={0.8}
                  >
