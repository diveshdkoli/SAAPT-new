...
//  Slides up from bottom. All fields editable. Saves directly to Firestore.
//  Email shown as read-only — Firebase Auth email change needs re-auth,
//  so we only update the Firestore display fields here.
// ══════════════════════════════════════════════════════════════════════════════
const EditUserSheet = ({ visible, user, onClose, onSaved }) => {
  const [name,        setName]        = useState('');
  const [phone,       setPhone]       = useState('');
  const [email,       setEmail]       = useState(''); // read-only display
  const [saving,      setSaving]      = useState(false);

  // Populate fields whenever the sheet opens with a new user
  useEffect(() => {
    if (user) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Update Firestore users/{uid} with changed fields
      // WHY only name/phone: email change in Firebase Auth requires
      // re-authentication which must be done by the user themselves,
      // not the admin. Firestore email field stays as-is.
      await updateDoc(doc(db, 'users', user.id), {
        name:      name.trim(),
        full_name: name.trim(),
        // WHY both: different parts of the app read either 'name' or 'full_name'
        phone:     phone.trim(),
      });

      // Propagate the update up to the list so UI refreshes immediately
      onSaved({
        ...user,
        name:      name.trim(),
        full_name: name.trim(),
      ...
      onClose();
      Alert.alert('✅ Updated', `${name.trim()} has been updated successfully.`);
    } catch (err) {
      console.error('EditUserSheet save error:', err);
  ...
  const role = ROLE_CONFIG[user.role] ?? {
    label: user.role ?? 'User', icon: '👤',
    color: COLORS.textSecondary, lightColor: COLORS.inputBg, initBg: '#9CA3AF',
  };
  ...
  return (
    ...
    >
      <View style={es.overlay}>
        ...
        <TouchableOpacity style={es.backdrop} activeOpacity={1} onPress={handleClose} />
        ...
        >
          <View style={es.sheet}>
            ...
            <View style={es.sheetHeader}>
              {/* Avatar + role badge */}
              <View style={[es.headerAvatar, { backgroundColor: role.initBg }]}>
                <Text style={es.headerAvatarTxt}>{getInitials(name || user.name || '?')}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={es.sheetTitle}>Edit Teacher</Text>
                <View style={[es.roleBadge, { backgroundColor: role.lightColor }]}>
                  <Text style={es.roleIcon}>{role.icon}</Text>
                  <Text style={[es.roleLbl, { color: role.color }]}>{role.label}</Text>
                </View>
              </View>
              {/* Close button */}
              <TouchableOpacity style={es.closeBtn} onPress={handleClose} activeOpacity={0.7}>
            ...
            >
              ...
              <View style={es.fieldGroup}>
                <Text style={es.fieldLabel}>
                  Full Name <Text style={es.required}>*</Text>
                </Text>
                <View style={es.inputRow}>
                  <Text style={es.inputIcon}>👤</Text>
                  <TextInput
                    style={es.input}
                    value={name}
              ...
              <View style={es.fieldGroup}>
                <Text style={es.fieldLabel}>
                  Email Address
                  <Text style={es.readOnlyNote}> (cannot be changed here)</Text>
                </Text>
                <View style={[es.inputRow, es.inputRowReadOnly]}>
                  <Text style={es.inputIcon}>📧</Text>
                  <Text style={es.readOnlyValue} numberOfLines={1}>{email || '—'}</Text>
                  <View style={es.lockBadge}>
                    <Text style={es.lockBadgeTxt}>🔒</Text>
                  </View>
                </View>
                <Text style={es.readOnlyHint}>
                  Email is linked to Firebase Auth. Teacher must change it themselves via account settings.
                </Text>
              </View>
              ...
              <View style={es.fieldGroup}>
                <Text style={es.fieldLabel}>
                  Phone Number
                  <Text style={es.optionalNote}> (optional)</Text>
                </Text>
                <View style={es.inputRow}>
                  <Text style={es.inputIcon}>📞</Text>
                  <TextInput
              ...
              {/* ── Info Box ── */}
              <View style={es.infoBox}>
                <Text style={es.infoTxt}>
                  ℹ️ Changes save directly to the database. Name and phone can be updated here.
                  To change email or password, the teacher must use account settings or Forgot Password.
                </Text>
              </View>
              ...
              >
                {saving
                  ...
                  : <Text style={es.saveBtnTxt}>✓  Save Changes</Text>
                }
              </TouchableOpacity>
...
const es = StyleSheet.create({
  overlay:    { flex: 1, backgroundColor: 'rgba(30,27,75,0.55)', justifyContent: 'flex-end' },
  ...
  sheet: {
    ...
    paddingTop: 12,
    maxHeight: '90%',
    // elevation shadow so it lifts above backdrop
    elevation: 24,
    shadowColor: '#000',
  ...
  headerAvatarTxt: { color: '#fff', fontWeight: '900', fontSize: 18 },
  sheetTitle:  { fontSize: 20, fontWeight: '800', color: COLORS.text, marginBottom: 6, letterSpacing: -0.3 },
  roleBadge:   { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3, gap: 4 },
  roleIcon:    { fontSize: 12 },
  roleLbl:     { fontSize: 12, fontWeight: '700' },
  closeBtn:    { width: 34, height: 34, borderRadius: 11, backgroundColor: COLORS.inputBg, alignItems: 'center', justifyContent: 'center' },
  closeBtnTxt: { fontSize: 13, color: COLORS.textSecondary, fontWeight: '700' },

  // Fields
  fieldGroup:  { marginBottom: 18 },
  fieldLabel:  { fontSize: 13, fontWeight: '700', color: COLORS.text, marginBottom: 8 },
  required:    { color: COLORS.danger },
  optionalNote:{ color: COLORS.textLight, fontWeight: '400', fontSize: 12 },
  readOnlyNote:{ color: COLORS.textLight, fontWeight: '400', fontSize: 11 },
  ...
  inputRowReadOnly: {
    // slightly different styling for non-editable email row
  ...
  readOnlyValue: { flex: 1, fontSize: 14, color: COLORS.textLight, paddingVertical: 12 },
  lockBadge:  { backgroundColor: '#E9ECF0', borderRadius: 8, paddingHorizontal: 7, paddingVertical: 3 },
  lockBadgeTxt:{ fontSize: 12 },
  readOnlyHint:{ fontSize: 11, color: COLORS.textLight, marginTop: 5, marginLeft: 2, lineHeight: 16 },
  ...
  saveBtnDisabled: { opacity: 0.65, elevation: 0 },
...
const ExcelPreviewScreen = ({ teachers: initialTeachers, onBack, onCommit, committing }) => {
  const [teachers,   setTeachers]   = useState(initialTeachers.map((t, i) => ({ ...t, _key: String(i) })));
  const [editingKey, setEditingKey] = useState(null);
  const [editVals,   setEditVals]   = useState({});

  const startEdit = (teacher) => { setEditingKey(teacher._key); setEditVals({ ...teacher }); };
  const saveEdit  = () => {
    setTeachers(prev => prev.map(t => t._key === editingKey ? { ...editVals, _key: editingKey } : t));
    setEditingKey(null);
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ padding: 16 }}>
          <View style={modalStyles.infoBox}>
            <Text style={modalStyles.infoTxt}>📝 Tap ✏️ to edit. Tap 🗑️ to remove. Then tap Commit.</Text>
          </View>
              ...
              {editingKey === teacher._key ? (
                ...
                <View style={styles.previewRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {!!teacher.phone && <Text style={styles.previewMeta}>📞 {teacher.phone}</Text>}
                  </View>
                  <View style={{ flexDirection: 'row', gap: 6 }}>
                    <TouchableOpacity style={[styles.previewAction, { backgroundColor: COLORS.primaryLight }]} onPress={() => startEdit(teacher)} activeOpacity={0.7}><Text>✏️</Text></TouchableOpacity>
                    <TouchableOpacity style={[styles.previewAction, { backgroundColor: COLORS.dangerLight }]}  onPress={() => removeTeacher(teacher._key)} activeOpacity={0.7}><Text>🗑️</Text></TouchableOpacity>
                  </View>
                </View>
              )}
            </View>
          ...
          >
            {committing
              ...
              : <Text style={modalStyles.saveBtnText}>✅ Commit {teachers.length} Teacher{teachers.length !== 1 ? 's' : ''} to Database</Text>
            }
          </TouchableOpacity>
...
const AddUserModal = ({ visible, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    if (!name.trim())                          { Alert.alert('Required', 'Please enter a full name.'); return; }
    if (!email.trim() || !email.includes('@')) { Alert.alert('Required', 'Please enter a valid email address.'); return; }
    setSaving(true);
    await onSave({ name: name.trim(), email: email.trim().toLowerCase(), phone: phone.trim(), role: 'teacher' });
    setSaving(false);
    reset();
  };
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={modalStyles.formGroup}>
                ...
                <View style={modalStyles.inputRow}>
                  ...
                  <TextInput style={modalStyles.input} placeholder="e.g. John Smith" placeholderTextColor={COLORS.textLight} value={name} onChangeText={setName} autoCapitalize="words" />
                </View>
              </View>
              ...
              <View style={modalStyles.formGroup}>
                ...
                <View style={modalStyles.inputRow}>
                  <Text style={modalStyles.inputIcon}>📧</Text>
                  <TextInput style={modalStyles.input} placeholder="e.g. teacher@school.com" placeholderTextColor={COLORS.textLight} value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} />
                </View>
              </View>
              ...
              <View style={modalStyles.formGroup}>
                ...
                <View style={modalStyles.inputRow}>
                  <Text style={modalStyles.inputIcon}>📞</Text>
                  <TextInput style={modalStyles.input} placeholder="+91 9876543210" placeholderTextColor={COLORS.textLight} value={phone} onChangeText={setPhone} keyboardType="phone-pad" />
                </View>
              </View>

              <View style={modalStyles.infoBox}>
                <Text style={modalStyles.infoTxt}>🔒 Password will be set to <Text style={{ fontWeight: '800' }}>saapt123456</Text> automatically.</Text>
              </View>
                ...
                <View style={[modalStyles.previewCard, { borderColor: COLORS.primary + '40' }]}>
                  ...
                  <View style={modalStyles.previewInfo}>
                    ...
                    <View style={[modalStyles.previewBadge, { backgroundColor: COLORS.primaryLight }]}>
                      <Text style={[modalStyles.previewBadgeText, { color: COLORS.primary }]}>👩‍🏫  Teacher</Text>
                    </View>
                  </View>
                </View>
              )}
              ...
              >
                {saving ? <ActivityIndicator color="#fff" size="small" /> : <Text style={modalStyles.saveBtnText}>✓  Add Teacher</Text>}
              </TouchableOpacity>
              <View style={{ height: 32 }} />
            </ScrollView>
          </View>
  ...
  return (
    <View style={styles.userCard}>
      ...
      <View style={styles.cardActions}>
        {/* Edit — now opens bottom sheet instead of navigating */}
        <TouchableOpacity style={[styles.actionBtn, styles.editBtn]} onPress={() => onEdit(user)} activeOpacity={0.7}>
  ...
  return (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtitle}>
        {isFiltered ? 'Try a different search or filter' : 'Add your first teacher to get started'}
      </Text>
    </View>
  );
};
...
const UsersScreen = ({ navigation }) => {
  ...
  // ── Edit sheet state ───────────────────────────────────────────────────────
  const [editSheetUser,    setEditSheetUser]    = useState(null);  // user being edited
  ...
  const handleAddUser = async ({ name, email, phone, role }) => {
    try {
      ...
      const credential = await createUserWithEmailAndPassword(secondaryAuth, email, DEFAULT_TEACHER_PASSWORD);
  ...
  // ── Open edit sheet ────────────────────────────────────────────────────────
  // Called from UserCard's ✏️ button — opens the bottom sheet
  const handleEdit = (user) => {
    setEditSheetUser(user);
    setEditSheetVisible(true);
  };

  // ── After successful save in edit sheet ────────────────────────────────────
  // Receives the updated user object → patches local state so list refreshes
  // without needing a full Firestore re-fetch
  ...
  const handleExcelPick = async () => {
    ...
    try {
      ...
      const fileContent = await FileSystem.readAsStringAsync(result.assets[0].uri, { encoding: 'base64' });
      const workbook = xlsxRead(fileContent, { type: 'base64' });
      const rows = xlsxUtils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]]);

      if (rows.length === 0) { Alert.alert('Empty File', 'No data found in the Excel file.'); return; }
      ...
      if (parsed.length === 0) { Alert.alert('No Valid Teachers', 'No rows with both name and email. Check column names.'); return; }
  ...
  const handleExcelCommit = async (teachers) => {
    ...
    for (const teacher of teachers) {
      try {
        const secondaryAuth = getSecondaryAuth();
        const credential = await createUserWithEmailAndPassword(secondaryAuth, teacher.email, DEFAULT_TEACHER_PASSWORD);
        const uid = credential.user.uid;
        await secondaryAuth.signOut();
  ...
  if (showPreview) {
    return <ExcelPreviewScreen teachers={excelTeachers} onBack={() => setShowPreview(false)} onCommit={handleExcelCommit} committing={committing} />;
  }
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity style={styles.headerAddBtn} onPress={() => setExcelInstructions(true)} activeOpacity={0.8}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.tabsRow}>
          {FILTER_TABS.map((tab) => {
            ...
            return (
              <TouchableOpacity key={tab.key} style={[styles.tab, isActive && styles.tabActive]} onPress={() => setSelectedFilter(tab.key)} activeOpacity={0.75}>
        ...
        {!loading && (
          <View style={styles.resultsMeta}>
            <Text style={styles.resultsText}>{filteredUsers.length} {filteredUsers.length === 1 ? 'user' : 'users'} found</Text>
          </View>
        )}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <UserCard
                ...
                onEdit={handleEdit}   // ← opens bottom sheet now
              />
            )}
      ...
      {/* Add Teacher Modal */}
      <AddUserModal visible={modalVisible} onClose={() => setModalVisible(false)} onSave={handleAddUser} />

      {/* Excel Instructions */}
      <ExcelInstructionsModal visible={excelInstructions} onClose={() => setExcelInstructions(false)} onProceed={handleExcelPick} />

      {/* ── Edit User Bottom Sheet ── */}
      <EditUserSheet
        visible={editSheetVisible}
        user={editSheetUser}
        onClose={() => { setEditSheetVisible(false); setEditSheetUser(null); }}
...
const modalStyles = StyleSheet.create({
  ...
  inputRow:   { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.inputBg, borderRadius: 14, paddingHorizontal: 14, borderWidth: 1.5, borderColor: COLORS.border },
  inputIcon:  { fontSize: 15, marginRight: 10 },
  input:      { flex: 1, fontSize: 15, color: COLORS.text, paddingVertical: 13 },
  previewCard:{ flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.inputBg, borderRadius: 14, padding: 14, marginBottom: 20, borderWidth: 1.5, gap: 12 },
  previewAvatar: { width: 46, height: 46, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  ...
  saveBtn:    { backgroundColor: COLORS.primary, borderRadius: 16, paddingVertical: 15, alignItems: 'center', shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.30, shadowRadius: 10, elevation: 6 },
  saveBtnDisabled: { opacity: 0.65 },
  saveBtnText:{ color: '#FFFFFF', fontSize: 16, fontWeight: '700', letterSpacing: 0.3 },
  colRow:     { flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginBottom: 10, paddingVertical: 4 },
  colBadge:   { borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3, minWidth: 64, alignItems: 'center' },
  ...
  infoBox:    { backgroundColor: COLORS.warningLight, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: COLORS.warning, marginBottom: 12 },
...
const styles = StyleSheet.create({
  ...
  header:     { backgroundColor: COLORS.primary, paddingTop: 52, paddingBottom: 20, paddingHorizontal: 20, borderBottomLeftRadius: 28, borderBottomRightRadius: 28 },
  ...
  headerAddBtn:  { backgroundColor: 'rgba(255,255,255,0.22)', borderRadius: 20, paddingVertical: 8, paddingHorizontal: 16, borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.35)' },
  ...
  tab:           { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.card, borderRadius: 12, paddingVertical: 9, paddingHorizontal: 8, gap: 6, borderWidth: 1.5, borderColor: COLORS.border },
  ...
  userCard:      { backgroundColor: COLORS.card, borderRadius: 18, padding: 14, flexDirection: 'row', alignItems: 'center', shadowColor: COLORS.shadow, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 },
  ...
  previewHeader: { backgroundColor: COLORS.primary, paddingTop: 52, paddingHorizontal: 16, paddingBottom: 18, flexDirection: 'row', alignItems: 'center', gap: 12 },
  previewBackBtn:{ width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.35)' },
  ...
  previewCard:   { backgroundColor: COLORS.card, borderRadius: 14, padding: 14, marginBottom: 10, elevation: 2 },
  previewRow:    { flexDirection: 'row', alignItems: 'center', gap: 12 },
  previewAvatar: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  ...
  previewEditInput:  { backgroundColor: COLORS.inputBg, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, color: COLORS.text, borderWidth: 1.5, borderColor: COLORS.border },
  previewAction:     { width: 34, height: 34, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
