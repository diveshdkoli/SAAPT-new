...
const ClassesScreen = ({ user }) => {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [clSnap, uSnap] = await Promise.all([
        getDocs(query(collection(db,'classes'), where('created_by','==',adminId), orderBy('createdAt','desc'))),
        getDocs(query(collection(db,'users'), where('created_by','==',adminId))),
      ]);
  ...
  useEffect(() => { if (adminId) fetchAll(); }, [adminId]);
  const onRefresh = useCallback(() => { setRefreshing(true); fetchAll(); }, [adminId]);
  ...
  const handleCreateClass = async (name) => {
    try {
      ...
      setClasses(prev => [{id:ref.id, className:name, students:[], created_by:adminId, createdAt:new Date()}, ...prev]);
  ...
  const handleAddStudent = async (studentData) => {
    try {
      ...
      const userDoc = {
        ...
        role:               'student',
        uid,
        created_by:         adminId,   // ← partition key
        createdAt:          serverTimestamp(),
      };
