...
export default function StudentReportScreen() {
  ...
  // the published defaulter doc for student's class (null if none published)
  const [isOnDefaulterList, setIsOnDefaulterList] = useState(false);
  // true if THIS student appears in the published list
  const [showDefaulterList, setShowDefaulterList] = useState(false);
  // toggle to show/hide the full published list view
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // ── STEP 1: Find student's class + fetch own batch ───────────────────
      ...
      // Fetch this student's own batch from their users doc
      // WHY: needed to filter practical sessions — only count sessions for my batch
      let myBatch = '';
      try {
        const myUserSnap = await getDoc(doc(db, 'users', uid));
        if (myUserSnap.exists()) {
          myBatch = myUserSnap.data().batch ?? '';
        }
      } catch (e) {
        console.log('Could not fetch student batch:', e);
      }
      ...
      attSnap.forEach(sessionDoc => {
        ...
        const sessionType = d.sessionType ?? 'lecture';
        const sessionBatch = d.batch ?? '';
        const records     = d.records     ?? [];

        // ── BATCH FILTER FOR PRACTICALS ──────────────────────────────────
        // WHY: if this is a practical session meant for a DIFFERENT batch,
        // skip it entirely — it doesn't count for this student at all
        // e.g. a batch B student should ignore all batch A and batch C sessions
        if (sessionType === 'practical') {
          if (myBatch && sessionBatch && sessionBatch.toUpperCase() !== myBatch.toUpperCase()) {
            return; // skip this session — wrong batch
          }
        }

        const myRecord = records.find(r => r.studentId === uid);
        if (!myRecord) return;
        // WHY: if student not in records, they weren't part of this session
      ...
      // ── STEP 5: Check if student is on published defaulter list ──────────
      // WHY: student needs to see a warning if admin published a defaulter list
      // that includes them
      try {
        const pubSnap = await getDoc(doc(db, 'published_defaulters', foundClassId));
        // WHY foundClassId as doc ID: we store one doc per class using classId
        if (pubSnap.exists()) {
          const pubData = pubSnap.data();
          setPublishedList(pubData);
          // Check if this student's UID is in the students array
          const onList = (pubData.students ?? []).some(s => s.uid === uid);
          setIsOnDefaulterList(onList);
        }
      } catch(e) {
        // published list not found — that's fine, not an error
        console.log('No published defaulter list for this class');
      }
    ...
    <View style={s.container}>
      ...
      >
        ...
        <View style={s.body}>
          ...
          {/* Shows when student is on a published defaulter list */}
          {isOnDefaulterList && (
            <View style={s.defaulterWarningBanner}>
              <Text style={s.defaulterWarningTitle}>
                🚨 You Are On The Defaulter List!
              </Text>
              <Text style={s.defaulterWarningBody}>
                Your class admin has published a defaulter list and you are included.
                Please improve your attendance immediately.
              </Text>
            </View>
          )}

          {/* ── Published Defaulter List Button ── */}
          {/* Shows when a published list exists for this student's class */}
          {publishedList && (
            <TouchableOpacity
              style={s.pubListBtn}
              onPress={() => setShowDefaulterList(v => !v)}
              activeOpacity={0.85}
            >
              <Text style={s.pubListBtnTxt}>
                📋 {showDefaulterList ? 'Hide' : 'View'} Class Defaulter List
              </Text>
              <Text style={s.pubListBtnArrow}>{showDefaulterList ? '▲' : '▼'}</Text>
            </TouchableOpacity>
          )}

          {/* ── Defaulter List (expanded) ── */}
          {showDefaulterList && publishedList && (
            <View style={s.pubListCard}>
              <Text style={s.pubListTitle}>
                ⚠️ Defaulter List — {publishedList.className}
              </Text>
              <Text style={s.pubListMeta}>
                {publishedList.fromDate} → {publishedList.toDate} · Below {publishedList.threshold}%
              </Text>
              {(publishedList.students ?? []).map((st, i) => (
                <View
                  key={st.uid}
                  style={[s.pubListRow, st.uid === auth.currentUser?.uid && s.pubListRowMe]}
                >
                  {/* Highlight current student's row */}
                  <Text style={s.pubListRowNum}>{i + 1}</Text>
                  <View style={{flex:1}}>
                    <Text style={s.pubListRowName}>
                      {st.uid === auth.currentUser?.uid ? '👉 ' : ''}{st.name}
                    </Text>
                  </View>
                  <View style={s.pubListRowBadge}>
                    <Text style={s.pubListRowPct}>{st.overallPct}%</Text>
                  </View>
                </View>
              ))}
            </View>
...
const s = StyleSheet.create({
  ...
  // Defaulter warning banner
  defaulterWarningBanner: { backgroundColor:'#FEF2F2', borderRadius:14, padding:14, marginBottom:14, borderWidth:2, borderColor:'#DC2626' },
  defaulterWarningTitle:  { fontSize:15, fontWeight:'800', color:'#DC2626', marginBottom:6 },
  defaulterWarningBody:   { fontSize:13, color:'#991B1B', lineHeight:20 },

  // Published list button
  pubListBtn:     { flexDirection:'row', alignItems:'center', justifyContent:'space-between', backgroundColor:'#EDE9FE', borderRadius:14, padding:14, marginBottom:14, borderWidth:1.5, borderColor:'#7C3AED' },
  pubListBtnTxt:  { fontSize:14, fontWeight:'700', color:'#7C3AED' },
  pubListBtnArrow:{ fontSize:14, color:'#7C3AED', fontWeight:'700' },

  // Defaulter list card
  pubListCard:  { backgroundColor:'#fff', borderRadius:16, padding:14, marginBottom:14, elevation:3, shadowColor:'#000', shadowOffset:{width:0,height:2}, shadowOpacity:0.06, shadowRadius:8 },
  pubListTitle: { fontSize:15, fontWeight:'800', color:'#1E1B4B', marginBottom:4 },
  pubListMeta:  { fontSize:12, color:'#6B7280', marginBottom:12 },
  pubListRow:   { flexDirection:'row', alignItems:'center', paddingVertical:10, borderTopWidth:1, borderTopColor:'#F3E8FF', gap:10 },
  pubListRowMe: { backgroundColor:'#FEF2F2', borderRadius:8, paddingHorizontal:6, borderLeftWidth:3, borderLeftColor:'#DC2626' },
  pubListRowNum:  { fontSize:12, color:'#9CA3AF', width:24 },
  pubListRowName: { fontSize:14, fontWeight:'600', color:'#1E1B4B' },
  pubListRowBadge:{ backgroundColor:'#FEE2E2', borderRadius:8, paddingHorizontal:8, paddingVertical:3 },
  pubListRowPct:  { fontSize:12, fontWeight:'800', color:'#DC2626' },
});
