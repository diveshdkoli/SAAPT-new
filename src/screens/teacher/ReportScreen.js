    ...
    <View style={[styles.studentRow, isDefaulter && styles.studentRowDefaulter]}>
      ...
      <View style={styles.studentMeta}>
        ...
        <Text style={styles.sessionText}>
          {student.roll_number ? `Roll: ${student.roll_number}  ·  ` : ''}{present} / {total} sessions
        </Text>
      </View>
...
const ReportsScreen = ({ navigation }) => {
  ...
  const [pubView,          setPubView]          = useState('report');
  // 'report' = normal report, 'published' = published lists, 'publishedDetail' = detail
  ...
  const buildReport = useCallback(async (classObj) => {
    ...
    try {
      ...
      const studentList = studentIds
        .map((id) => studentMap[id] ?? { id, name: 'Unknown', roll_number: '' })
        .sort((a, b) => {
  ...
  const calculateDefaulters = async () => {
    ...
    try {
      ...
      await Promise.all(studentUids.map(async (uid) => {
        try {
          ...
          if (!uSnap.empty) {
            ...
            nameMap[uid] = { name: uid, batch: '', roll_number: '' };
          }
        } catch {
          nameMap[uid] = { name: uid, batch: '', roll_number: '' };
        }
      ...
      const results = studentUids.map(uid => {
        ...
        // WHY read batch: filter practical sessions to only this student's batch
        ...
        // Filter practical sessions to only this student's batch
        ...
        // Count practicals this student was present for (batch-filtered)
        ...
        const practicalPct = pctNull(practicalPresent, myTotalPracticals);
        // WHY myTotalPracticals: denominator = sessions for this student's batch only
        ...
        return {
          ...
          roll_number:     nameMap[uid]?.roll_number ?? '',
          lecturePresent,
          totalLectures,
          lecturePct,
          practicalPresent,
      ...
      // Sort defaulters by roll number
  ...
  const exportPublishedPDF = async (pub) => {
    try {
      ...
      const html = `<html><head><meta charset="utf-8"/>
        ...
        <p>${pub.className} · ${fmtDate(pub.fromDate)} → ${fmtDate(pub.toDate)} · Below ${pub.threshold}%</p>
        <table><thead><tr><th>#</th><th>Name</th><th>Lectures</th><th>Practicals</th><th>Overall</th></tr></thead>
        <tbody>${rows}</tbody></table>
        <p style="font-size:11px;color:#9CA3AF;text-align:center;margin-top:24px;">SAAPT Attendance Management System</p>
        </body></html>`;
      const { uri } = await Print.printToFileAsync({ html, base64: false });
      await Sharing.shareAsync(uri, { mimeType:'application/pdf', dialogTitle:'Save Defaulter List', UTI:'com.adobe.pdf' });
    } catch(e) {
  ...
  if (pubView === 'publishedDetail' && selectedPub) {
    return (
      <View style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={COLORS.primary} />
        <View style={styles.header}>
          <TouchableOpacity style={styles.backBtn}
            onPress={() => { setPubView('published'); setSelectedPub(null); }} activeOpacity={0.8}>
            <Text style={styles.backBtnIcon}>←</Text>
          </TouchableOpacity>
          <View style={styles.headerTextGroup}>
            <Text style={styles.headerTitle}>{selectedPub.className}</Text>
            <Text style={styles.headerSubtitle}>
              {fmtDate(selectedPub.fromDate)} → {fmtDate(selectedPub.toDate)}
            </Text>
          </View>
          <TouchableOpacity
            onPress={() => exportPublishedPDF(selectedPub)}
            activeOpacity={0.8}
            style={{
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:16}}>
          <View style={{backgroundColor:COLORS.card, borderRadius:16, overflow:'hidden', elevation:3}}>
            <View style={{flexDirection:'row', backgroundColor:COLORS.primary, paddingVertical:10, paddingHorizontal:12}}>
              <Text style={{flex:0.4, fontSize:11, fontWeight:'800', color:'#fff'}}>#</Text>
              <Text style={{flex:2,   fontSize:11, fontWeight:'800', color:'#fff'}}>Name</Text>
              <Text style={{flex:1,   fontSize:11, fontWeight:'800', color:'#fff', textAlign:'center'}}>📖 Lec</Text>
              <Text style={{flex:1,   fontSize:11, fontWeight:'800', color:'#fff', textAlign:'center'}}>🔬 Prac</Text>
              <Text style={{flex:1,   fontSize:11, fontWeight:'800', color:'#fff', textAlign:'center'}}>Overall</Text>
            </View>
            {selectedPub.students.map((st, i) => (
              <View key={st.uid} style={{flexDirection:'row', paddingVertical:10, paddingHorizontal:12, alignItems:'center', backgroundColor: i%2===1 ? '#F9FAFB' : '#fff'}}>
                <Text style={{flex:0.4, fontSize:13, color:COLORS.textLight}}>{i+1}</Text>
                <Text style={{flex:2, fontSize:13, fontWeight:'600', color:COLORS.text}} numberOfLines={1}>{st.name}</Text>
                <Text style={{flex:1, textAlign:'center', fontSize:13, color:COLORS.primary}}>{st.lecturePct !== null ? `${st.lecturePct}%` : '—'}</Text>
                <Text style={{flex:1, textAlign:'center', fontSize:13, color:'#7C3AED'}}>{st.practicalPct !== null ? `${st.practicalPct}%` : '—'}</Text>
                <View style={{flex:1, alignItems:'center'}}>
                  <View style={{backgroundColor:COLORS.dangerLight, borderRadius:8, paddingHorizontal:8, paddingVertical:3}}>
                    <Text style={{fontSize:12, fontWeight:'800', color:COLORS.danger}}>{st.overallPct}%</Text>
                  </View>
                </View>
              </View>
            ))}
  ...
  // ── Published lists view ─────────────────────────────────────────────────
  if (pubView === 'published') {
    return (
      <View style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={COLORS.primary} />
        <View style={styles.header}>
          <TouchableOpacity style={styles.backBtn}
            onPress={() => setPubView('report')} activeOpacity={0.8}>
            <Text style={styles.backBtnIcon}>←</Text>
          </TouchableOpacity>
          <View style={styles.headerTextGroup}>
            <Text style={styles.headerTitle}>📋 Published Lists</Text>
            <Text style={styles.headerSubtitle}>All published defaulter lists</Text>
          </View>
        </View>
        {loadingPublished ? (
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:16}}>
            {publishedLists.length === 0 ? (
              <View style={styles.emptyCard}>
                ...
                <Text style={styles.emptyTitle}>No Published Lists Yet</Text>
                <Text style={styles.emptySubText}>Admin hasn't published any defaulter lists yet.</Text>
              </View>
            ) : (
              publishedLists.map(pub => (
                <TouchableOpacity key={pub.id}
                  style={{backgroundColor:COLORS.card, borderRadius:16, padding:16, marginBottom:12, flexDirection:'row', alignItems:'center', elevation:3}}
                  onPress={() => { setSelectedPub(pub); setPubView('publishedDetail'); }}
                  activeOpacity={0.82}
                >
                  <View style={{flex:1}}>
                    <Text style={{fontSize:16, fontWeight:'700', color:COLORS.text, marginBottom:4}}>{pub.className}</Text>
                    <Text style={{fontSize:12, color:COLORS.textSecondary}}>{fmtDate(pub.fromDate)} → {fmtDate(pub.toDate)}</Text>
                    <Text style={{fontSize:11, color:COLORS.textLight, marginTop:2}}>Below {pub.threshold}%</Text>
                  </View>
                  <View style={{backgroundColor:COLORS.dangerLight, borderRadius:10, paddingHorizontal:10, paddingVertical:6, alignItems:'center', marginRight:8}}>
                    <Text style={{fontSize:18, fontWeight:'900', color:COLORS.danger}}>{pub.students.length}</Text>
                    <Text style={{fontSize:9, color:COLORS.danger, fontWeight:'600'}}>Defaulters</Text>
                  </View>
    ...
    <View style={styles.container}>
      ...
      <View style={{ paddingHorizontal: 20, paddingBottom: 16 }}>
        <TouchableOpacity
          ...
          onPress={() => { setPubView('published'); loadPublishedLists(); }}
          activeOpacity={0.8}
        >
          <Text style={{ color: '#4F46E5', fontWeight: '800', fontSize: 14 }}>
            📋 View Published Defaulter Lists
          </Text>
          <Text style={{ color: '#4F46E5', fontSize: 20, fontWeight: '700' }}>›</Text>
        </TouchableOpacity>
      </View>
